 ...
 */
export function createBtimeAdapter(BaseAdapter) {
  return class BtimeAdapter extends BaseAdapter {
    meta = {
      id: "btime",
      name: "北京时间号",
      icon: "https://www.btime.com/favicon.ico",
      homepage: "https://mp.btime.com/",
    ...
    async probeAuth() {
      const urls = [
        "https://mp.btime.com/api/user/info",
        "https://mp.btime.com/api/account/info",
        "https://mp.btime.com/mp/user/info",
        "https://user.btime.com/api/user/info",
      ];
        ...
        try {
          ...
          if (
            ...
            return {
              ...
              username: String(username || "北京时间号"),
      ...
      try {
        const response = await this.runtime.fetch("https://mp.btime.com/", {
          credentials: "include",
        ...
        if (
          /\/login|passport|申请认证|扫码登录|请登录/i.test(
            finalUrl + html.slice(0, 1500),
          ) &&
          !/写文章|创作中心|文章管理/i.test(html)
        ) {
          // auth page without creator shell
          if (/申请认证|未认证/i.test(html) && !/写文章|发布/i.test(html)) {
            return null;
          }
          if (/\/login|passport/i.test(finalUrl)) return null;
        }
        if (/时间号|写文章|创作|草稿|文章管理/i.test(html)) {
          return { userId: "session", username: "北京时间号" };
        }
      } catch {
        // ignore
      }
      ...
      try {
        ...
        if (!account) {
          return {
            ...
            error:
              "未登录北京时间号。请打开 https://mp.btime.com/ 登录并完成时间号认证",
          };
        }
    ...
    async saveViaApi(title, content) {
      const payloads = [
        {
          url: "https://mp.btime.com/api/article/draft",
          body: { title, content, status: "draft" },
        },
        {
          url: "https://mp.btime.com/api/article/save",
          body: { title, content, draft: 1 },
        },
        {
          url: "https://mp.btime.com/mp/article/save",
      ...
      return { ok: false, error: "时间号草稿接口不可用" };
    }
      ...
      try {
          ...
          .executeScript({
            ...
            func: () => {
              ...
              const hit = nodes.find((n) =>
                /写文章|发文章|新建|创作图文/.test((n.textContent || "").trim()),
              );
              if (hit) {
                hit.dispatchEvent(
        ...
        const [{ result } = {}] = await chrome.scripting.executeScript({
          ...
          func: async (payload) => {
            ...
            if (/login|passport|申请认证/i.test(location.href + document.title)) {
              return {
                ok: false,
                error: "请先登录并完成时间号认证后再同步",
              };
            }
    ...
    async publish(article, options) {
      try {
        ...
        if (!auth.isAuthenticated) {
          throw new Error(auth.error || "未登录北京时间号");
        }
        ...
        if (dom.ok) {
          return this.createResult(true, {
            postUrl: dom.postUrl || "https://mp.btime.com/",
            draftOnly: options?.draftOnly ?? true,
            message: dom.message || api.error,
          });
        }

        throw new Error(dom.error || api.error || "时间号保存草稿失败");
      } catch (error) {
