/**
 * MD5 hex digest. WebCrypto does not support MD5; Zhihu image tokens need it.
...
export function md5Hex(input) {
  ...
  while (words.length < n) words.push(0);
  const bitLen = bytes.length * 8;
  words[n - 2] = bitLen | 0;
  words[n - 1] = Math.floor(bitLen / 0x100000000);
...
function cmnHex(n) {
  let out = "";
  for (let i = 0; i < 4; i += 1) {
    out += ((n >> (i * 8 + 4)) & 15).toString(16);
    out += ((n >> (i * 8)) & 15).toString(16);
  }
  return out;
}
