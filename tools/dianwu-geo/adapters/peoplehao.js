 ...
 */
export function createPeoplehaoAdapter(BaseAdapter) {
  return class PeoplehaoAdapter extends BaseAdapter {
    meta = {
      id: "peoplehao",
      name: "人民号",
      icon: "https://www.people.com.cn/favicon.ico",
      homepage: "https://pdcreator.pdnews.cn/producer",
    ...
    async probeAuth() {
      const urls = [
        "https://pdcreator.pdnews.cn/producerapi/user/info",
        "https://pdcreator.pdnews.cn/producerapi/account/info",
        "https://pdcreator.pdnews.cn/producermp/user/info",
        ...
        try {
          const response = await this.runtime.fetch(url, {
            ...
            headers: {
              ...
              Referer: "https://pdcreator.pdnews.cn/producer",
            },
          });
      ...
      try {
        const response = await this.runtime.fetch("https://pdcreator.pdnews.cn/producer", {
          credentials: "include",
        ...
        if (/人民号|写文章|创作|草稿|文章管理/i.test(html)) {
          return { userId: "session", username: "人民号" };
        }
      } catch {
        // ignore
      }
      ...
      try {
        ...
        if (!account) {
          return {
            ...
            error: "未登录人民号。请打开 https://pdcreator.pdnews.cn/ 用人民日报/视界 App 扫码登录并完成入驻",
          };
        }
    ...
    async saveViaApi(title, content) {
      const payloads = [
        {
          url: "https://pdcreator.pdnews.cn/producerapi/article/draft",
          body: { title, content, status: "draft" },
        },
        {
          url: "https://pdcreator.pdnews.cn/producerapi/article/save",
          body: { title, content, draft: 1 },
        },
        {
          url: "https://pdcreator.pdnews.cn/producermp/article/save",
        ...
        try {
          const response = await this.runtime.fetch(url, {
            ...
            headers: {
              ...
              Referer: "https://pdcreator.pdnews.cn/producer",
              Origin: "https://pdcreator.pdnews.cn",
            },
          ...
          if (ok) {
            return {
              ...
              postUrl: "https://pdcreator.pdnews.cn/producer",
            };
          }
        } catch {
          // next
      ...
      return { ok: false, error: "人民号草稿接口不可用" };
    }
    ...
    async saveViaDom(title, content) {
      ...
      try {
        const tabs = await chrome.tabs.query({
          url: ["*://pdcreator.pdnews.cn/*", "*://pdnews.cn/*"],
        });
        let tab =
        ...
        if (!tab?.id) {
          tab = await chrome.tabs.create({
            url: "https://pdcreator.pdnews.cn/producer",
            active: true,
          });
    ...
    async publish(article, options) {
      try {
        ...
        if (!auth.isAuthenticated) {
          throw new Error(auth.error || "未登录人民号");
        }
        ...
        if (dom.ok) {
          return this.createResult(true, {
            postUrl: dom.postUrl || "https://pdcreator.pdnews.cn/producer",
            draftOnly: options?.draftOnly ?? true,
            message: dom.message || api.error,
          });
        }

        throw new Error(dom.error || api.error || "人民号保存草稿失败");
      } catch (error) {
