 ...
 */
export function createZhongqingAdapter(BaseAdapter) {
  return class ZhongqingAdapter extends BaseAdapter {
    meta = {
      id: "zhongqing",
      name: "中青号",
      icon: "https://www.cyol.com/favicon.ico",
      homepage: "https://mp.cyol.com/",
    ...
    async probeAuth() {
      const urls = [
        "https://mp.cyol.com/api/user/info",
        "https://mp.cyol.com/api/account/info",
        "https://mp.cyol.com/mp/user/info",
      ...
      try {
        const response = await this.runtime.fetch("https://mp.cyol.com/", {
          credentials: "include",
        ...
        if (/中青号|写文章|创作|草稿|文章管理/i.test(html)) {
          return { userId: "session", username: "中青号" };
        }
      } catch {
        // ignore
      }
      ...
      try {
        ...
        if (!account) {
          return {
            ...
            error: "未登录中青号。请打开 https://mp.cyol.com/ 登录并完成创作者注册",
          };
        }
    ...
    async uploadImageByUrl(src) {
      return createHostOnlyUpload(["cyol.com","youth.cn"], "中青号")(src);
    }

    async saveViaApi(title, content) {
      const payloads = [
        {
          url: "https://mp.cyol.com/api/article/draft",
          body: { title, content, status: "draft" },
        },
        {
          url: "https://mp.cyol.com/api/article/save",
          body: { title, content, draft: 1 },
        },
        {
          url: "https://mp.cyol.com/mp/article/save",
      ...
      return { ok: false, error: "中青号草稿接口不可用" };
    }
    ...
    async publish(article, options) {
      try {
        ...
        if (!auth.isAuthenticated) {
          throw new Error(auth.error || "未登录中青号");
        }
        ...
        content = await processAndAssertImages(
          ...
          {
            skipPatterns: ["cyol.com", "youth.cn"],
        ...
        if (dom.ok) {
          return this.createResult(true, {
            postUrl: dom.postUrl || "https://mp.cyol.com/",
            draftOnly: options?.draftOnly ?? true,
            message: dom.message || api.error,
          });
        }

        throw new Error(dom.error || api.error || "中青号保存草稿失败");
      } catch (error) {
