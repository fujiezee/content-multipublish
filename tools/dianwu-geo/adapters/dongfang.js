 ...
 */
export function createDongfangAdapter(BaseAdapter) {
  return class DongfangAdapter extends BaseAdapter {
    meta = {
      id: "dongfang",
      name: "东方号",
      icon: "https://www.eastday.com/favicon.ico",
      homepage: "https://mp.eastday.com/",
    ...
    async probeAuth() {
      const urls = [
        "https://mp.eastday.com/api/user/info",
        "https://mp.eastday.com/api/account/info",
        "https://mp.eastday.com/mp/user/info",
      ...
      try {
        const response = await this.runtime.fetch("https://mp.eastday.com/", {
          credentials: "include",
        ...
        if (/东方号|写文章|创作|草稿|文章管理/i.test(html)) {
          return { userId: "session", username: "东方号" };
        }
      } catch {
        // ignore
      }
      ...
      try {
        ...
        if (!account) {
          return {
            ...
            error: "未登录东方号，请打开 https://mp.eastday.com/ 登录",
          };
        }
    ...
    async uploadImageByUrl(src) {
      return createHostOnlyUpload(["eastday.com","tt.cn"], "东方号")(src);
    }

    async saveViaApi(title, content) {
      const payloads = [
        {
          url: "https://mp.eastday.com/api/article/draft",
          body: { title, content, status: "draft" },
        },
        {
          url: "https://mp.eastday.com/api/article/save",
          body: { title, content, draft: 1 },
        },
        {
          url: "https://mp.eastday.com/mp/article/save",
      ...
      return { ok: false, error: "东方号草稿接口不可用" };
    }
    ...
    async publish(article, options) {
      try {
        ...
        if (!auth.isAuthenticated) {
          throw new Error(auth.error || "未登录东方号");
        }
        ...
        content = await processAndAssertImages(
          ...
          {
            skipPatterns: ["eastday.com", "tt.cn"],
        ...
        if (dom.ok) {
          return this.createResult(true, {
            postUrl: dom.postUrl || "https://mp.eastday.com/",
            draftOnly: options?.draftOnly ?? true,
            message: dom.message || api.error,
          });
        }

        throw new Error(dom.error || api.error || "东方号保存草稿失败");
      } catch (error) {
